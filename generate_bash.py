...
with open("create_users_hourly.sh", "w") as cb:
    for f in sorted(Path("page/keys_hourly").iterdir()):
        ...
        if datetime.datetime.fromisoformat(f.stem[: f.stem.rfind("-")]) > datetime.datetime.fromisoformat("2020-10-17"):
            cb.write(
                f"echo parsing {f}; python ./create_users.py -m 1 -v v1.5 -d {f} > page/users_hourly/{f.stem}.txt\n"
            )

        elif datetime.datetime.fromisoformat(f.stem[: f.stem.rfind("-")]) > datetime.datetime.fromisoformat(
            "2020-07-02"
        ):
